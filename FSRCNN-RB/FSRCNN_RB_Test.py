...
if __name__ == '__main__':
    ...
    model = FSRCNN_RB(scale_factor=inp['scale']).to(device)  # GPU
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(['--weights-file', 'C:\\Users/Mehrgan/PycharmProjects/Article 2/Asli/FSRCNN_RB/output/best.pth',
                                            '--image-file', 'C:\\Users/Mehrgan/PycharmProjects/Article 2/FSRCNN_RB/3/Image Folder/'])

    cudnn.benchmark = True
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    model = FSRCNN_RB(scale_factor=args.scale).to(device)
