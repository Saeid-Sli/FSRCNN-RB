...
PNGarray_New_Downsample = np.zeros((20000, 64, 64)).astype(np.float)
print('***** DownSampling *****')
for j in range (0, 20000):
    PNGarray_New_Downsample[j] = cv2.pyrDown(PNGarray_New[j])

print('dtype Images After DownSampling: ', PNGarray_New_Downsample.dtype)
print('Shape Images After DownSampling: ', PNGarray_New_Downsample.shape)

print(f'{PNGarray_New_Downsample.shape[0]} subimages with a size of {(64,64)} were generated.')

print('*** Saving h5 database ***')
# Saving the h5 file
h5py_name = "LR_Q01.h5"
hf = h5py.File(h5py_name, 'w')
hf.create_dataset('3D_image', data=PNGarray_New_Downsample)
