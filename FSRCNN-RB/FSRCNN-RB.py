...
if __name__ == '__main__':
    inp = {
        ...
        'outputs-dir': 'C:\\Users/Mehrgan/PycharmProjects/Article 2/Article1/FSRCNN_RB/output/',
        'scale': 4,
        'learning_rate': 1e-3,
        'batch_size': 5,
        'num_epochs': 100,
    ...
    chkp_Path = f'./Results/8.{DatafolderName[:-3]}_{inp["optimizer"]}_b{inp["batch_size"]}_lr{inp["learning_rate"]}_LR2HR-model3'
    pathlib.Path(chkp_Path).mkdir(parents=True, exist_ok=True)

    t = time.time()

    model = FSRCNN_RB().to(device)  # GPU
    ...
    optimizer = optim.Adam([
        ...
        {'params': model.conv1_2.parameters()},
        ...
        {'params': model.layer9.parameters()},
        {'params': model.layer10.parameters()},
        {'params': model.layer11.parameters()},
    #     {'params': model.last_layer.parameters()}
    # ], lr=inp['learning_rate'])
    {'params': model.last_layer.parameters(), 'learning_rate': inp['learning_rate'] * 0.1}
    ], lr = inp['learning_rate'])
   ...
   #=============================================================================
    ...
    Loss_Training = np.zeros(100)
    PSNR = np.zeros(100)
    Epoch_elapsed_time = np.zeros(100)
    t_total = time.time()
    ...
    for epoch in range(start_epoch, inp['num_epochs']):
        ...
        with tqdm(total=(len(Train_Dataset) - len(Train_Dataset) % inp['batch_size']), position=0, leave=True) as t:
            ...
            for x, y in Eval_dataloader:
                ...
                epoch_psnr_eval.update(calc_psnr(preds, y), len(x))

            # print('Eval PSNR: {:.2f}'.format(epoch_psnr.avg))
            table = [["PSNR", epoch_psnr_eval.avg]]
            print(tabulate(table, headers=["\nPeak Signal-to-Noise ratio for Validation", "\nValue of PSNR"]))
            ...
            for x, y in Test_dataloader:
                with torch.no_grad():
                    preds_test = model(x).clamp(0.0, 1.0)
                    # preds_SSIM = model(x)
                    # score = structural_similarity(preds_SSIM, y)

                epoch_psnr_test.update(calc_psnr(preds_test, y), len(x))

            # print('Eval PSNR: {:.2f}'.format(epoch_psnr.avg))
            table = [["PSNR", epoch_psnr_test.avg]]
            print(tabulate(table, headers=["\nPeak Signal-to-Noise ratio for Test", "\nValue of PSNR"]))
            # print(tabulate([["\nPSNR"], ["Eval PSNR", epoch_psnr.avg], ["loss_avg", loss_avg]], headers = "firstrow"))

            print(Fore.BLUE + '==================================================================================================================================')
            PSNR[epoch] = epoch_psnr_test.avg
            # print("Image similarity: {}".format(score))

            if epoch_psnr_test.avg > best_psnr:
                best_epoch = epoch
                best_psnr = epoch_psnr_test.avg
                best_weights = copy.deepcopy(model.state_dict())
